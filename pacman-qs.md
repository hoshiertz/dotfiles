...
local/alsa-card-profiles 1:1.4.6-1
...
local/appstream 1.0.5-2
    Provides a standard for creating app stores across distributions

local/archlinux-keyring 20250707-1
    Arch Linux PGP keyring

local/at-spi2-core 2.56.3-1
...
local/binutils 2.44+r94+gfe459e33c676-1
...
local/device-mapper 2.03.33-1
    Device mapper userspace library and tools

local/dhcpcd 10.2.4-1
...
local/gcc 15.1.1+r7+gf36ec88aa85a-1
    The GNU Compiler Collection - C and C++ frontends

local/gcc-libs 15.1.1+r7+gf36ec88aa85a-1
...
local/glibc 2.41+r48+g5cb575ca9a3d-1
    GNU C Library

local/glslang 1:1.4.313.0-1
...
local/gst-plugins-bad-libs 1.26.3-2
    Multimedia graph framework - bad

local/gst-plugins-base-libs 1.26.3-2
    Multimedia graph framework - base

local/gstreamer 1.26.3-2
...
local/harfbuzz 11.2.1-1
    OpenType text shaping engine

local/harfbuzz-icu 11.2.1-1
...
local/jdk-openjdk 24.0.2.u12-1
...
local/kitty 0.42.1-1
    A modern, hackable, featureful, OpenGL-based terminal emulator

local/kitty-shell-integration 0.42.1-1
    Shell integration scripts for kitty, an OpenGL-based terminal emulator

local/kitty-terminfo 0.42.1-1
...
local/lib32-freetype2 2.13.3-3
...
local/lib32-libtheora 1.2.0-1
    Standard encoder and decoder library for the Theora video compression format (32-bit)
...
local/lib32-libunwind 1.8.1-1
    Determine and manipulate the call-chain of a program (32-bit)

local/lib32-libusb 1.0.29-1
...
local/lib32-libxcrypt-compat 4.4.38-1
    Modern library for one-way hashing of passwords - legacy API functions (32bit)
...
local/lib32-openssl-1.1 1.1.1.w-2
    The Open Source toolkit for Secure Sockets Layer and Transport Layer Security

local/lib32-orc 0.4.41-1
    Optimized Inner Loop Runtime Compiler (32-bit)

local/lib32-p11-kit 0.25.5-1
...
local/lib32-pipewire 1:1.4.6-1
    Low-latency audio/video router and processor - 32-bit

local/lib32-speex 1.2.1-2
...
local/libtiff 4.7.0-1
    Library for manipulation of TIFF images

local/libtirpc 1.3.6-2
    Transport Independent RPC library (SunRPC replacement)

local/libtool 2.5.4+r23+g5b582aed-1
...
local/libxcrypt-compat 4.4.38-1
...
local/libxkbcommon 1.10.0-1
    Keymap handling library for toolkits and window systems

local/libxkbcommon-x11 1.10.0-1
...
local/libxml2 2.14.4-2
    XML C parser and toolkit

local/libxmlb 0.3.22-1
...
local/python-more-itertools 10.7.0-1
    More routines for operating on iterables, beyond itertools

local/python-packaging 25.0-1
    Core utilities for Python packages

local/python-pillow 11.3.0-1
    Python Imaging Library (PIL) fork

local/python-platformdirs 4.3.6-2
    A library to determine platform-specific system directories

local/python-setuptools 1:80.9.0-1 (python-build-backend)
    Easily download, build, install, upgrade, and uninstall Python packages

local/python-vdf 4.0-2
...
local/startup-notification 0.12-8
    Libary to monitor and display application startup

local/steam 1.0.0.82-2
...
local/systemd 257.7-1
    system and service manager

local/systemd-libs 257.7-1
    systemd client libraries

local/systemd-sysvcompat 257.7-1
...
local/v2ray 5.30.0-1
    A platform for building proxies to bypass network restrictions

local/v2ray-domain-list-community 20250627153051-1
    A list of domains to be used as geosites for routing purpose in Project V

local/v2ray-geoip 202507050144-1
    GeoIP List for V2Ray

local/v2raya 2.2.6.7-1
    A web GUI client of Project V which supports VMess, VLESS, SS, SSR, Trojan, Tuic and Juicity protocols

local/v2raya-debug 2.2.6.7-1
    Detached debugging symbols for v2raya

local/v4l-utils 1.30.1-1
    Userspace tools and conversion library for Video 4 Linux

local/vapoursynth R70-2
    A video processing framework with the future in mind

local/vid.stab 1.1.1-2
    Video stabilization library

local/vim 9.1.1552-1
    Vi Improved, a highly configurable, improved version of the vi text editor

local/vim-runtime 9.1.1552-1
...
local/vscodium-bin 1.102.04606-1
    Binary releases of VS Code without MS branding/telemetry/licensing.

local/vscodium-bin-debug 1.102.04606-1
    Detached debugging symbols for vscodium-bin
...
local/wxwidgets-gtk3 3.2.8.1-2
...
local/yarn 1.22.22-2
    Fast, reliable, and secure dependency management

local/yyjson 0.11.1-1
    A high performance JSON library written in ANSI C

local/zenity 4.1.90-1
